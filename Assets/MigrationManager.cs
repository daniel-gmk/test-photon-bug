...
public class MigrationManager : MonoBehaviour, INetworkRunnerCallbacks
{
    ...
    {
        INetworkObjectProvider originalProvider = runner.ObjectProvider;
        INetworkSceneManager originalSceneManager = runner.SceneManager;

        if (runner.TryGetSceneInfo(out var originalSceneInfo) == false)
        {
            Debug.LogError("Error on HostMigration. Original scene info is not valid.");
            return;
        }
        ...
        StartGameResult result = await runner.StartGame(new StartGameArgs()
        {
            ...
            ObjectProvider = originalProvider,
            Scene = originalSceneInfo,
    ...
    void HostMigrationResume(NetworkRunner runner)
    {
        ...
        {
            sceneNO.Item1.CopyStateFrom(sceneNO.Item2);
    ...
    public static unsafe PlayerRef FromByteArray(byte[] token)
    {
        ...
        pv[2] = token[2];
        pv[3] = token[3];
        return PlayerRef.FromIndex(value - 1);
    }
