...
public class FusionHelper : MonoBehaviour, INetworkRunnerCallbacks
{
    ...
    {
        if (runner.IsServer && !GameManager.Instance.started)
        {
    ...
    public void OnShutdown(NetworkRunner runner, ShutdownReason shutdownReason)
    {
        ...
        switch (shutdownReason)
        {
            ...
            case ShutdownReason.HostMigration:
                launcher.SetConnectionStatus(FusionLauncher.ConnectionStatus.Migrating, "Migrating Host");
                return;
            default:
                message = shutdownReason.ToString();
                break;
        }
        launcher.SetConnectionStatus(FusionLauncher.ConnectionStatus.Disconnected, message);
